...
#include <tensorpipe/transport/ib/context.h>

#include <tensorpipe/common/system.h>
#include <tensorpipe/transport/ib/connection.h>
#include <tensorpipe/transport/ib/listener.h>
...
std::shared_ptr<Context> makeIbContext() {
  return std::make_shared<Context>();
}

TP_REGISTER_CREATOR(TensorpipeTransportRegistry, ib, makeIbContext);

// Prepend descriptor with transport name so it's easy to
// disambiguate descriptors when debugging.
const std::string kDomainDescriptorPrefix{"ib:"};
...
class Context::Impl : public Context::PrivateIface,
 ...
 public:
  ...
  IbvProtectionDomain& getIbvPd() override;

  IbvCompletionQueue& getIbvCq() override;

  IbvSharedReceiveQueue& getIbvSrq() override;
  ...
  void registerQp(
      ...
      std::function<void(uint32_t)> onProduced) override;

  void unregisterQp(uint32_t qpn) override;
...
IbvContext& Context::Impl::getIbvContext() {
  return reactor_.getIbvContext();
}

IbvProtectionDomain& Context::Impl::getIbvPd() {
  return reactor_.getIbvPd();
}

IbvCompletionQueue& Context::Impl::getIbvCq() {
  return reactor_.getIbvCq();
}

IbvSharedReceiveQueue& Context::Impl::getIbvSrq() {
  return reactor_.getIbvSrq();
}

IbvAddress& Context::Impl::getIbvAddress() {
  return reactor_.getIbvAddress();
}

void Context::Impl::registerQp(
    uint32_t qpn,
    std::function<void(uint32_t)> onConsumed,
    std::function<void(uint32_t)> onProduced) {
  reactor_.registerQp(qpn, std::move(onConsumed), std::move(onProduced));
}

void Context::Impl::unregisterQp(uint32_t qpn) {
  reactor_.unregisterQp(qpn);
}
