...
// FIXME Turn this back into a macro?
template <typename T>
T* TP_CHECK_IBV_PTR(T* ptr) {
...
inline uint32_t ibvMtuToInt32(enum ibv_mtu mtu) {
  switch (mtu) {
    case IBV_MTU_256:
      return 256;
    case IBV_MTU_512:
    ...
    case IBV_MTU_4096:
      return 4096;
    default:
      TP_THROW_ASSERT() << "Unknown MTU value: " << mtu;
  }
}

inline enum ibv_mtu ibvInt32ToMtu(uint32_t mtu) {
  switch (mtu) {
    case 256:
      return IBV_MTU_256;
    case 512:
  ...
  // Fake return to make the compiler happy.
  return IBV_MTU_256;
}
